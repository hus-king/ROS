...
{
	ros::init(argc, argv, "deliver_goods");
	ros::NodeHandle nh("~");
	...
	while (ros::ok())
	{
		...
		if (deliver == 1)
		{
			if (point_flag == 1)  //飞到A点
			{
				...
				if (abs_distance < 0.2 && !is_arrived)
				{
					is_arrived = true;
					cout << "arrived A point" << endl;
				}
				if (is_arrived)
				{					
					count++;
			...
			else if (point_flag == 0)
			{
				...
				float abs_distance;
				abs_distance = sqrt((pos_drone.pose.position.x) * (pos_drone.pose.position.x) + (pos_drone.pose.position.y) * (pos_drone.pose.position.y));
				if (abs_distance < 0.1 && flag_land == 0)
				{
					flag_land = 1;
					cout << "arrived land are" << endl;
				}
			}
		}
