...
void camera_callback(const ros::TimerEvent&) {
    ...
    // 将深度帧转换为OpenCV Mat并应用颜色映射
    rs2::frame depth_colorized = color_map.process(depth_frame);
    depth_image = cv::Mat(cv::Size(w, h), CV_8UC3, (void*)depth_colorized.get_data(), cv::Mat::AUTO_STEP);
...
int determine_drone_direction() {
    ...
    for (int x = 0; x < w; ++x) {
        int depth_index = 0;
        for (int y = 0; y < h; ++y) {
            float depth_value = depth_image.at<unsigned short>(y, x) * 0.001f;  // 假设深度图是以毫米为单位
