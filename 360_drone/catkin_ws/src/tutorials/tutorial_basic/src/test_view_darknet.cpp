...
int main(int argc, char **argv) {
    ros::init(argc, argv, "view_darknet_node");
    ros::NodeHandle nh;
                 ...
                 if(find_target){
                      ...
                      ROS_ERROR("lean_angle:%f",lean_angle);
                        if(fabs(lean_angle) >= angle_thres)  //YAML 面向框目标角15°
                        {
                            ...
                            vision_target.x = current_pose.pose.position.x + (dz-0.3)*cos(current_rpy.z) - dy*sin(current_rpy.z);
                            vision_target.y = current_pose.pose.position.y + (dz-0.3)*sin(current_rpy.z) + dy*cos(current_rpy.z);
