...
geometry_msgs::Twist get_pid_vel(geometry_msgs::Point target,float reach_yaw) {
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "vel_control_node");
    ros::NodeHandle nh;
        ...
        switch (fsm_state) {
            ...
            case 1:  // After offboard, before armed state
                if (current_state.armed) {
                    fsm_state = 2;
                    now_position.x = current_pose.pose.position.x;
                    now_position.y = current_pose.pose.position.y;
                    now_position.z = 1.1;      // goto takeoff state
                    now_yaw = current_rpy.z;
                    } else {
                    if (ros::Time::now() - last_srv_request > ros::Duration(1.0)) {
            ...
            case 2:  // Takeoff state
                if (current_pose.pose.position.z > 1.0) {
                    ...
                    twist.twist = get_pid_vel(now_position,now_yaw);
                }
                break;
            case 3:  // Move state
                ...
                if (ros::Time::now() - last_srv_request > ros::Duration(3.0)) {
                    ...
                    now_position.x = current_pose.pose.position.x;
                    now_position.y = current_pose.pose.position.y;
                    now_position.z = 0.1;      // goto takeoff state
                    now_yaw = current_rpy.z;  // goto land state
                } else {  // PID control
                    twist.twist = get_pid_vel(hover_target,now_yaw);
                }
                break;
            case 4:  // Land state
