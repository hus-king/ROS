...
void TimeSurvel();
bool VetiJudge(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg,int bigbox,int chessbox1,int chessbox2);
...
void darknet_box_cb(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg){
    ...
    if(no_target_time>flip_mode_thresh){
        ROS_ERROR("compare chessboard");
        ...
        ROS_WARN("Potential target there but match failed!ID:%d",flag0);
    }
    }
...
void ABConstructor(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg,int bigbox,int smlbox1,int smlbox2,int chessbox1,int chessbox2){
    ...
    TARGET.only_css=false;TARGET.onematch=false;
    TARGET.cssbox[0]=msg->bounding_boxes[chessbox1];
    TARGET.cssbox[1]=msg->bounding_boxes[chessbox2];
    if(VetiJudge(msg,bigbox,chessbox1,chessbox2)){
        TARGET.flip_css=true;
    }
    else{
        TARGET.flip_css=false;
    }
    return;
}
bool VetiJudge(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg,int bigbox,int chessbox1,int chessbox2){
    if(msg->bounding_boxes[bigbox].id==3){
      if(abs(msg->bounding_boxes[bigbox].ymin-msg->bounding_boxes[chessbox1].ymin)>0.2*abs(msg->bounding_boxes[bigbox].ymin-msg->bounding_boxes[bigbox].ymax)){
            return true;
        }
        else if(abs(msg->bounding_boxes[bigbox].ymin-msg->bounding_boxes[chessbox2].ymin)>0.2*abs(msg->bounding_boxes[bigbox].ymin-msg->bounding_boxes[bigbox].ymax)){
            return true;
        }
    }
    else{
...
float gain_depth(){
    ...
    if(TARGET.bigbox.id == 3){ //square
        if(TARGET.cssbox[0].id == 1){ //blue chess
            ...
            for(int j = 0;j < static_cast<int>(TARGET.cssbox[0].ymax - TARGET.cssbox[0].ymin)/2.0;j++){
                for(int i = 0 ;i < static_cast<int>( (TARGET.cssbox[0].xmax - TARGET.cssbox[0].xmin)/2.0 ) ; i++) {
                    if(depth_msg[static_cast<int>(x_middle + i)][static_cast<int>(y_middle+j)]<=min_thres || depth_msg[static_cast<int>(x_middle + i)][static_cast<int>(y_middle+j)]>=far_thres) {}
                    else{   
        ...
        if(TARGET.cssbox[0].id == 7){ //blue cur
            ...
            for(int j = 0;j < static_cast<int>(TARGET.cssbox[0].ymax - TARGET.cssbox[0].ymin)/2.0;j++) {
                for(int i = 0 ;i < static_cast<int>( (TARGET.cssbox[0].xmax - TARGET.cssbox[0].xmin)/2.0 ) ; i++) {
                    if(depth_msg[static_cast<int>(x_tmiddle + i)][static_cast<int>(y_middle+j)]<=min_thres || depth_msg[static_cast<int>(x_tmiddle + i)][static_cast<int>(y_middle+j)]>=far_thres) {}
                    else{   
                        depth_sum += depth_msg[static_cast<int>(x_middle + i)][static_cast<int>(y_middle+j)];
                        e_index ++;
                        }
...
int main(int argc, char **argv) {
    ...
    nh.param<int>("rand_move_thresh", rand_move_thresh, 500);
    ...
    nh.param<int>("flip_mode_thresh",flip_mode_thresh,250);
    nh.param<float>("min_thres",min_thres,1.5f);
            ...
            case 3:  // Navigation state
               if (target_index >= hover_target.points.size()) {
                ...
                if (getLengthBetweenPoints(hover_target.points[target_index], current_pose.pose.position) < 0.1) //到达预设点位
                {
                    ...
                    twist.twist = get_pid_vel(hover_target.points[target_index],hover_target.yaw[target_index]);
                }
                /*if(find_target){
                   set_now_position.x = current_pose.pose.position.x;
                   set_now_position.y = current_pose.pose.position.y;
            ...
            case 5: //finding_state 寻找目标框
                 ...
                 if(no_target_time > rand_move_thresh){
                     if(finding_state == 1){
                        ...
                        vision_target.z = current_pose.pose.position.z + 0.5;//方形移动路线上方点
						missed_target=false;
        				missed_time=0;
       					no_target_time=0;
                 ...
                 if(find_target){
                    ROS_ERROR("find target --- ready to crossing");
                    
                      twist.twist = get_pid_vel(set_now_position,set_now_yaw); 
                       ROS_ERROR("completed pid velocity calculation");
                      CalculateCoordinate();
                      ROS_ERROR("completed coordinate calculation");
                      if(fabs(z1-z2)>=1.0) 
                      {
                        twist.twist = get_pid_vel(set_now_position,set_now_yaw);
                         ROS_ERROR("depth msgs error---------");
                          vel_pub.publish(twist);
                        ros::spinOnce();
                         rate.sleep();
                         continue;
                      ...
                      ROS_ERROR("lean_angle:%f",lean_angle);
                        if(fabs(lean_angle) >= angle_thres)  //YAML 面向框目标角15°
                        {
                            ...
                            vision_target.x = current_pose.pose.position.x + (dz+0.7)*cos(current_rpy.z) - dy*sin(current_rpy.z);
                            vision_target.y = current_pose.pose.position.y + (dz+0.7)*sin(current_rpy.z) + dy*cos(current_rpy.z);
            ...
            case 64: //up or down to view box 上下移动识别方框
                if (getLengthBetweenPoints(vision_target, current_pose.pose.position) >= 0.1) {
                   twist.twist = get_pid_vel(vision_target,set_now_yaw);
                    ...
                    std::cout<<"finished moving up , arrived back target point"<<std::endl;
                    ROS_ERROR("-----------------------------------------");
                    fsm_state = 5;  //TODO set 5
                    missed_target = false;
                    missed_time = 0;
